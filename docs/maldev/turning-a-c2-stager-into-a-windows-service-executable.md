...
* `Executed by Services.exe (SCM):` Executed by the Windows OS itself through the Service Control Manager (SCM)
...
| Endpoint Product w/ default config           | C:\Windows\System32 | C:\Program Files | C:\Program Files (x86) |
| -------------------------------------------- | :-----------------: | :--------------: | :--------------------: |
| CrowdStrike Falcon Insight XDR               |          x          |         x        |            x           |
| SentinelOne Singularity Complete / ActiveEDR |          x          |         x        |            x           |
| Palo Alto Cortex XDR Pro                     |          x          |         x        |            x           |
| Elastic Defend                               |          x          |         x        |            x           |
| Microsoft Defender for Endpoint              |          x          |         x        |            x           |
| Trellix ENS / EDR                            |          x          |                  |                        |
| Wazuh Windows FIM                            |                     |                  |                        |
| OSSEC Syscheck                               |                     |                  |                        |
...
`conhost.exe` with existing `conhost.exe` as parent

Final Result: `services.exe` -> `serviceExecutable.exe` ->-----_injection_----$\downarrow$ `conhost.exe` -> `conhost.exe (C2)`
...
`WmiPrvSE.exe` with existing `svchost.exe` as parent

Final Result: `services.exe` -> `serviceExecutable.exe` ->-----_injection_----$\downarrow$ `svchost.exe` -> `WmiPrvSE.exe (C2)`
